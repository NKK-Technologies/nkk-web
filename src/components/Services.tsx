        ...
        <div className="grid gap-3.5 md:gap-5 md:grid-cols-2 lg:grid-cols-4">
          {services.map(({ icon: Icon, title, body }, i) => (
            <Reveal key={title} delay={i * 80}>
              <article className="group h-full flex items-start gap-4 md:flex-col md:gap-3.5 bg-white border border-line rounded-2xl shadow-card p-5 md:p-6 transition-[translate,box-shadow] duration-200 ease-[var(--ease-standard)] hover:-translate-y-1 hover:shadow-[0_4px_8px_rgba(10,25,47,.1),0_12px_32px_rgba(10,25,47,.14)]">
                <span className="shrink-0 flex items-center justify-center w-11 h-11 rounded-xl bg-ice transition-[scale,background-color] duration-200 ease-standard group-hover:scale-[1.08] group-hover:bg-[#D2E9F7]">
                  <Icon />
                </span>
